...
namespace PS3Lib
{
    public class CCAPI
    {
        ...
        [DllImport("kernel32.dll", SetLastError = true)]
        static extern IntPtr LoadLibraryEx(string dllName);
        [DllImport("kernel32.dll", SetLastError = true)]
        ...
        //private SafeHINSTANCE safeModule;
        private List<IntPtr> CCAPIFunctionsList = [];
            ...
            if (File.Exists(DllUrl))
            {
                ...
                if (libModule != IntPtr.Zero)
                {
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("LastErrorCode = " + GetLastError());
                        MessageBox.Show("Il y a " + CCAPIFunctionsList.Count + " Objets chargés avec comme première valeur :" + CCAPIFunctionsList[0]);
                    }
                }
                else
                {
                    MessageBox.Show("Impossible to load the DLL CCAPI 2.60+", "CCAPI.dll cannot be loaded", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    MessageBox.Show("LastErrorCode = " + GetLastError());
                }
            }
            else
